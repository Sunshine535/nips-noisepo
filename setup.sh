#!/usr/bin/env bash
# NeurIPS 2026 — nips-noisepo: conda env, PyTorch 2.4.0 + CUDA 12.1, deps, flash-attn, verify
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
cd "$ROOT"

ENV_NAME="${NIPS_CONDA_ENV:-nips-noisepo}"
PY_VER="${PYTHON_VERSION:-3.10}"

echo "============================================"
echo " nips-noisepo — environment setup"
...
for mod in ("transformers", "trl", "peft", "datasets", "accelerate", "scipy"):
